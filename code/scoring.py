...
# domain for this function should be [0, 1], if score go up, machine generation faith go up
def get_ratio_logprob_score(real_text, generated_text):
    real_score = get_log_probability(real_text)
    generated_score = get_log_probability(generated_text)
    return np.divide(real_score, generated_score)

# domain for this function should be [1, inf]
def get_likelihood(ratio):
    return np.exp(ratio)
